...
export default function LeaderboardTopStores({
  items,
  demoMode,
}: {
  ...
  return (
    ...
    >
      <CardHeader className="flex items-center justify-between mb-1">
        <div className="text-sm font-medium text-white/80">Top Stores — Revenue</div>
        {demoMode ? (
      ...
      <CardContent>
        {isLoading ? (
            ...
            {Array.from({ length: 5 }).map((_, i) => (
              <li key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="h-3 w-44 sm:w-52 rounded bg-white/10" />
                </div>
          ...
          <ul className={`space-y-3 mt-2 transition-all duration-300 ease-out ${revealed ? "opacity-100 translate-y-0" : "opacity-0 translate-y-1"}`}>
            {items.map((it, idx) => (
              <li key={it.storeId} className="flex items-center justify-between" data-testid="store-row">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="truncate">
                    <div className="text-white/90 truncate" title={it.storeName} data-testid="store-name">{it.storeName}</div>
                    <div className="text-[11px] text-white/60 truncate" data-testid="store-loc">
                      {[it.city, it.state].filter(Boolean).join(', ')}
                    </div>
                  </div>
                </div>
                <div className="text-white/85 font-mono text-sm" data-testid="store-revenue">{fmtCurrency(it.revenue)}</div>
              </li>
            ))}
          </ul>
        )}
        <p className="sr-only">Ordered list of top stores by revenue.</p>
      </CardContent>
    </Card>
  );
}
